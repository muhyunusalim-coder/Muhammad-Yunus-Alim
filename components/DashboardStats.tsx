...
const StatCard = ({ title, value, icon: Icon, color, subtext, trend, delay }: { title: string, value: string, icon: any, color: 'blue' | 'amber' | 'emerald', subtext: string, trend?: string, delay: string }) => {
    ...
    return (
        <div className={`flex flex-col h-full relative overflow-hidden bg-white/70 backdrop-blur-xl rounded-3xl md:rounded-[2.5rem] p-6 md:p-8 shadow-xl ${theme.shadow} border border-white/40 group hover:-translate-y-1 hover:shadow-2xl transition-all duration-500 animate-in fade-in slide-in-from-bottom-8 ${delay}`}>
            ...
            <div className="relative z-10 flex flex-col flex-1">
                ...
                <div className="flex flex-col flex-1">
                    ...
                    <div className="flex items-center gap-2 pt-4 border-t border-slate-50/50 mt-auto">
                        <span className={`w-2 h-2 rounded-full bg-gradient-to-r ${theme.gradient} block shrink-0`}></span>
